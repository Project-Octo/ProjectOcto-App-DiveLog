...
class _DivesPageState extends State<DivesPage> {
  late Future<List> _futureOfDiveRecords;
  late List diveRecords = [];
  late List<bool> _isExpandedList = [];

  @override
  void initState() {
    super.initState();
    _futureOfDiveRecords = fetchData().then((value) {
      diveRecords = value;
      _isExpandedList = List.generate(value.length, (index) => false);
      return value;
    });
  ...
  Future<List> fetchData() async {
    ...
    final snapshot = await FirebaseFirestore.instance
        ...
        .get();
    final List diveRecords = snapshot.docs.map((doc) => doc.data()).toList();
        ...
        builder: (BuildContext context, AsyncSnapshot snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            if (snapshot.hasError)
              ...
              if (diveRecords.isEmpty) {
                return const Center(
                  child: Text('No dive records yet!'),
                );
              }
              return Container(
                      ...
                      child: Container(
                        ...
                        child: ExpansionTile(
                          ...
                          shape: RoundedRectangleBorder(
                            borderRadius:
                                BorderRadius.circular(10.0), // 타일의 모서리를 라운드 처리
                          ),
                          subtitle: Column(
                            //본문
                            crossAxisAlignment: CrossAxisAlignment.start,
